...
func newIPN(jsConfig js.Value) map[string]any {
	...
	netM, err := netmon.New(logf)
	if err != nil {
		log.Fatalf("netmon.New: %v", err)
	}

	srv := ipnserver.New(logf, logid, netM)
	...
	return map[string]any{
		...
		"put": js.FuncOf(func(this js.Value, args []js.Value) any {
			if len(args) != 3 {
				log.Printf("Usage: put(url, type, body)")
				return nil
			}
			...
			return jsIPN.put(url, tp, body)
		}),
		"post": js.FuncOf(func(this js.Value, args []js.Value) any {
			if len(args) != 3 {
				log.Printf("Usage: post(url, type, body)")
				return nil
			}
...
func (i *jsIPN) put(url string, ype string, body string) js.Value {
	return makePromise(func() (any, error) {
		...
		req.Header.Set("Content-Type", ype)
		res, err := c.Do(req)
		if err != nil {
			return nil, err
		}
...
func (i *jsIPN) post(url string, ype string, body string) js.Value {
	return makePromise(func() (any, error) {
		c := &http.Client{
			Transport: &http.Transport{
				DialContext: i.dialer.UserDial,
		...
		res, err := c.Post(url, ype, strings.NewReader(body))
		if err != nil {
			return nil, err
		}
